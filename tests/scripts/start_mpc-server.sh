...
RUST_BACKTRACE=full $MPC_SERVER_REPO/target/debug/mpc-server -s secret/p4.s --port 8004 -m http://127.0.0.1:8000  --db-path db/p4.db > log/p4.log 2>&1 &
RUST_BACKTRACE=full $MPC_SERVER_REPO/target/debug/mpc-server -s secret/p5.s --port 8005 -m http://127.0.0.1:8000  --db-path db/p5.db > log/p5.log 2>&1 &
RUST_BACKTRACE=full $MPC_SERVER_REPO/target/debug/mpc-server -s secret/p6.s --port 8006 -m http://127.0.0.1:8000  --db-path db/p6.db > log/p6.log 2>&1 &
RUST_BACKTRACE=full $MPC_SERVER_REPO/target/debug/mpc-server -s secret/p7.s --port 8007 -m http://127.0.0.1:8000  --db-path db/p7.db > log/p7.log 2>&1 &
