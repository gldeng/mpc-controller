...
func (t *Recovery) run(ctx core.TaskContext) ([]core.Task, error) {
	switch t.Status {
	case StatusInit:
...
func (t *Recovery) buildSignReq(id string, hash []byte) (*mpc.SignRequest, error) {
...
func (t *Recovery) buildAndSignTx(ctx core.TaskContext) error {
	...
	tx, err := builder.Recovery(t.Quorum.PubKey, amount, nonce)
	if err != nil {
...
func (t *Recovery) getSignature(ctx core.TaskContext) error {
...
//     at the same time
func (t *Recovery) sendTx(ctx core.TaskContext) error {
...
func (t *Recovery) checkIfTxIssued(ctx core.TaskContext) (bool, error) {
	status, err := t.checkTxStatus(ctx)
	if err != nil {
		t.logError(ctx, ErrMsgCheckTxStatusFail, err, []logger.Field{{"txId", t.TxID}}...)
...
func (t *Recovery) checkTxStatus(ctx core.TaskContext) (core.TxStatus, error) {
