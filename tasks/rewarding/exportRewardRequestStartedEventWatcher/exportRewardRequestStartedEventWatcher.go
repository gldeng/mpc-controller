package exportRewardRequestStartedEventWatcher
...
func (eh *ExportRewardRequestStartedEventWatcher) doWatchStakeRequestStarted(ctx context.Context) {
...
func (eh *ExportRewardRequestStartedEventWatcher) subscribeStakeRequestStarted(ctx context.Context, sink chan<- *contract.MpcManagerStakeRequestStarted, pubKeys [][]byte) error {
	if eh.sub != nil {
		eh.sub.Unsubscribe()
	}
	...
	go func() {
		for {
			...
			case <-eh.done:
				return
			case evt := <-eh.sink:
				evtObj := dispatcher.NewRootEventObject("ExportRewardRequestStartedEventWatcher", evt, ctx)
